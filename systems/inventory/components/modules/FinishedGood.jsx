...
export default function FinishedGood() {
  ...
  const fetchItems = async () => {
    try {
      ...
      const res = await inventoryApi.get('finished-goods', params);
      setItems(res.data || []);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to delete this finished good record?')) return;
    try {
      await inventoryApi.delete(`finished-goods/${id}`);
      fetchItems();
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await inventoryApi.put(`finished-goods/${editingItem.id}`, formData);
      } else {
        await inventoryApi.post('finished-goods', { ...formData, firmName: activeFirm });
      }
      setModalOpen(false);
      fetchItems();
    } catch (err) {
      alert(err.message || 'Failed to save finished good');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-white">Finished Goods Inventory</h1>
          <p className="text-sm text-zinc-500">Track production, sales dispatches, pending orders, and current stock level</p>
        </div>
        <button
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 w-full max-w-md space-y-4 shadow-xl">
            <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 pb-3">
              <h3 className="font-bold text-lg text-zinc-900 dark:text-white">
                {editingItem ? 'Edit Finished Good' : 'Add Finished Good'} ({activeFirm})
              </h3>
