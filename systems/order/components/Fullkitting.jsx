...
export default function Fullkitting() {
  ...
  const [form, setForm] = useState({ fullkittingStatus: "Yes", remarks: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API_URL}/order/fullkitting/${selected.id}`, {
        method: "POST",
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="flex items-end gap-4">
              <div><Label className="text-xs text-zinc-500 mb-1 block">Status</Label>
                <Select value={form.fullkittingStatus} onValueChange={v => setForm(f => ({ ...f, fullkittingStatus: v }))}>
                  <SelectTrigger className="h-9 w-32"><SelectValue /></SelectTrigger>
                  <SelectContent>{["Yes", "No"].map(v => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="flex-1"><Label className="text-xs text-zinc-500 mb-1 block">Remarks</Label><Input value={form.remarks} onChange={e => setForm(f => ({ ...f, remarks: e.target.value }))} className="h-9" /></div>
