...
import { 
  CheckCircle2, Search, ExternalLink, Eye, Check, X, ShieldAlert, AlertCircle 
} from 'lucide-react';
...
export default function PaymentApproval() {
  ...
  const loadData = async () => {
    try {
      ...
      console.error('Error loading payment approval data:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const approvalPayments = useMemo(() => {
    return payments.filter(p => {
      const matchesTab = activeTab === 'active'
        ? p.status === 'Channel Funded'
  ...
  const openApprovalModal = (p) => {
    setSelectedPayment(p);
    setRemarks(p.approverRemarks || p.remarks || '');
    setError('');
    setIsOpen(true);
  };
  ...
  const handleAction = async (nextStatus, isRejection = false) => {
    ...
    try {
      ...
      const res = await paymentApi.post(`requests/${selectedPayment.id}/approve`, {
        status: nextStatus,
  ...
  return (
    <div className="space-y-4 text-xs">
      ...
      <div className="border-b border-zinc-200 dark:border-zinc-800 pb-4">
        ...
        <p className="text-xs text-zinc-500 mt-1">Audit channel funded requisitions, verify supporting documents, and grant final management clearance.</p>
      </div>
      ...
      <div className="flex border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-1 rounded-xl border shadow-sm">
        ...
        >
          ...
          <span className="inline-flex items-center justify-center px-2 py-0.5 text-[10px] font-semibold rounded-full bg-purple-500 text-white">
            {payments.filter(p => p.status === 'Channel Funded').length}
          </span>
        </button>
        <button
        ...
        >
          ...
          <span className="inline-flex items-center justify-center px-2 py-0.5 text-[10px] font-semibold rounded-full bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-300">
            {payments.filter(p => ['Approved', 'Rejected'].includes(p.status)).length}
          </span>
        </button>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[950px]">
          <thead className="bg-zinc-50 dark:bg-zinc-950 uppercase text-[10px] text-zinc-400 font-bold border-b border-zinc-200 dark:border-zinc-800">
            <tr>
              ...
              <th className="px-5 py-3.5">Funding Channel</th>
            ...
            {approvalPayments.length === 0 ? (
              <tr>
                <td colSpan={activeTab === 'history' ? 8 : 9} className="text-center py-12 text-zinc-400 italic">
                  No payment approval requests found matching parameters.
                </td>
              </tr>
            ) : (
              approvalPayments.map(p => (
                <tr key={p.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/10">
                  <td className="px-5 py-3.5 font-mono font-bold text-purple-600 dark:text-purple-400">{p.paymentNumber}</td>
                  <td className="px-5 py-3.5">
                    <span className="inline-flex px-2 py-0.5 rounded border text-[10px] font-bold uppercase tracking-wide bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-400 border-purple-200">
                      {p.status}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-zinc-600 dark:text-zinc-400 font-semibold">{p.typeOfFunding || p.fundingChannel || '-'}</td>
                  ...
                  {activeTab !== 'history' && (
                    <td className="px-5 py-3.5 text-center">
                      <button
                        ...
                        className="px-3 py-1 bg-purple-50 dark:bg-purple-950/40 hover:bg-purple-100 dark:hover:bg-purple-900/60 text-purple-600 dark:text-purple-400 rounded-lg font-bold transition-colors inline-flex items-center gap-1"
                      >
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex justify-between items-center border-b border-zinc-100 dark:border-zinc-800 pb-3">
              <div>
                <h3 className="font-extrabold text-base text-zinc-900 dark:text-zinc-100">Audit &amp; Approve: {selectedPayment.paymentNumber}</h3>
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl space-y-1">
                <p><strong>Funding Type:</strong> {selectedPayment.typeOfFunding || 'N/A'}</p>
                <p><strong>Funding Remarks:</strong> {selectedPayment.fundingRemarks || 'None'}</p>
                <p><strong>Requisition Purpose:</strong> {selectedPayment.remarks || 'None'}</p>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-zinc-600 dark:text-zinc-400 mb-1">Approver Remarks / Authorization Note</label>
                <textarea
                  value={remarks}
                  onChange={(e) => setRemarks(e.target.value)}
                  rows={3}
                  placeholder="Notes regarding invoice verification, budget clearance, or approval..."
                  className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 text-xs outline-none focus:ring-1 focus:ring-purple-500"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-zinc-100 dark:border-zinc-800">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl text-xs shadow-sm transition-colors flex items-center gap-1.5"
                >
