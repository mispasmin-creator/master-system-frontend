...
import { 
  FileText, Search, ExternalLink, Eye, Check, X, Send, AlertCircle 
} from 'lucide-react';
...
export default function Posting() {
  ...
  const postingPayments = useMemo(() => {
    return payments.filter(p => {
      const matchesTab = activeTab === 'active'
        ? p.status === 'Approved'
        : (['Posted', 'Rejected'].includes(p.status));
  ...
  const openPostingModal = (p) => {
    setSelectedPayment(p);
    setRemarks(p.postingRemarks || p.remarks || '');
    setError('');
    setIsOpen(true);
  };
  ...
  const handleAction = async (nextStatus, isRejection = false) => {
    ...
    try {
      ...
      const res = await paymentApi.post(`requests/${selectedPayment.id}/post`, {
        status: nextStatus,
      ...
      if (res.success) {
        ...
        setError(res.error || "Failed to process posting.");
      }
    } catch (err) {
      setError(err.message || "Posting action failed.");
    } finally {
    ...
    <div className="space-y-4 text-xs">
      ...
      <div className="border-b border-zinc-200 dark:border-zinc-800 pb-4">
        <h1 className="text-xl font-bold text-zinc-900 dark:text-zinc-50 flex items-center gap-2">
          ...
          Accounting Posting &amp; Ledger Verification
        </h1>
        <p className="text-xs text-zinc-500 mt-1">Record accounting entry numbers, post approved requisitions into ERP/Tally, and advance to final disbursement.</p>
      </div>
      ...
      <div className="flex border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-1 rounded-xl border shadow-sm">
        ...
        >
          <span>Active Approved Payments</span>
          <span className="inline-flex items-center justify-center px-2 py-0.5 text-[10px] font-semibold rounded-full bg-sky-500 text-white">
        ...
        >
          ...
          <span className="inline-flex items-center justify-center px-2 py-0.5 text-[10px] font-semibold rounded-full bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-300">
            {payments.filter(p => ['Posted', 'Rejected'].includes(p.status)).length}
          </span>
        </button>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[950px]">
          <thead className="bg-zinc-50 dark:bg-zinc-950 uppercase text-[10px] text-zinc-400 font-bold border-b border-zinc-200 dark:border-zinc-800">
            <tr>
              ...
              <th className="px-5 py-3.5">Posting Remarks</th>
            ...
            {postingPayments.length === 0 ? (
              <tr>
                <td colSpan={activeTab === 'history' ? 8 : 9} className="text-center py-12 text-zinc-400 italic">
                  No posting payment requests found matching parameters.
                </td>
              </tr>
            ) : (
              postingPayments.map(p => (
                <tr key={p.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/10">
                  <td className="px-5 py-3.5 font-mono font-bold text-sky-600 dark:text-sky-400">{p.paymentNumber}</td>
                  <td className="px-5 py-3.5">
                    <span className="inline-flex px-2 py-0.5 rounded border text-[10px] font-bold uppercase tracking-wide bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-400 border-sky-200">
                      {p.status}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-zinc-500 max-w-[200px] truncate">{p.postingRemarks || p.remarks || '-'}</td>
                  ...
                  {activeTab !== 'history' && (
                    <td className="px-5 py-3.5 text-center">
                      <button
                        onClick={() => openPostingModal(p)}
                        className="px-3 py-1 bg-sky-50 dark:bg-sky-950/40 hover:bg-sky-100 dark:hover:bg-sky-900/60 text-sky-600 dark:text-sky-400 rounded-lg font-bold transition-colors inline-flex items-center gap-1"
                      >
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex justify-between items-center border-b border-zinc-100 dark:border-zinc-800 pb-3">
              <div>
                <h3 className="font-extrabold text-base text-zinc-900 dark:text-zinc-100">Confirm Posting: {selectedPayment.paymentNumber}</h3>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-[11px] font-bold text-zinc-600 dark:text-zinc-400 mb-1">Posting Voucher / Accounting Notes</label>
                <textarea
                  value={remarks}
                  onChange={(e) => setRemarks(e.target.value)}
                  rows={3}
                  placeholder="Enter Tally voucher no, ledger head, or posting confirmation details..."
                  className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 text-xs outline-none focus:ring-1 focus:ring-sky-500"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-zinc-100 dark:border-zinc-800">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => handleAction('Posted', false)}
                  disabled={actionLoading}
                  className="px-5 py-2 bg-sky-600 hover:bg-sky-700 text-white font-bold rounded-xl text-xs shadow-sm transition-colors flex items-center gap-1.5"
                >
                  <Send className="h-4 w-4" />
                  <span>Confirm Posting</span>
                </button>
              </div>
            </div>
