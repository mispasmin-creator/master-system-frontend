    ...
    <header className="bg-white shadow-sm p-3 sm:p-4 flex items-center justify-between sticky top-0 z-40 h-16 flex-shrink-0 border-b border-gray-200">
      ...
      <div className="flex items-center">
        ...
        <Button
          ...
          className="text-gray-600 hover:bg-green-50 hover:text-[#268a59] rounded-lg hidden md:inline-flex" // Hidden on mobile
        >
          <Menu size={24} />
        </Button>
        ...
        <Button
          ...
          className="text-gray-600 hover:bg-green-50 hover:text-[#268a59] rounded-lg md:hidden mr-4" // Visible on mobile
        >
          <Menu size={24} />
        </Button>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-[#268a59] focus:ring-opacity-50"
          >
            <UserCircle size={24} className="text-[#2fa36b]" />
            <div className="text-left hidden md:block">
              <p className="text-xs text-gray-500">Logged in as</p>
              <p className="text-sm font-medium text-[#268a59]">{user?.username || "User"}</p>
            </div>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent className="w-48 shadow-lg" align="end">
          <DropdownMenuLabel className="md:hidden">
            <p className="text-xs text-gray-500">Logged in as</p>
            <p className="text-sm font-semibold text-[#268a59] truncate">{user?.username || "User"}</p>
          </DropdownMenuLabel>
